...
var calculateRatio = function(passedLocation, male, female) {
  ...
  function getRecsWomenLength(passedLocation, male) {
    DummyProf.findOne({location: passedLocation, gender: male}, function(err, data) {
      if (err) throw err;

      tinderRecsWomen = data.tinderRecs.length;
      console.log('tinderRecsWomen: ' + tinderRecsWomen);
      
      getRecsMenLength(passedLocation, female);
    });
  }

  function getRecsMenLength(passedLocation, female) {
    DummyProf.findOne({location: passedLocation, gender: female}, function(err, data) {
      if (err) throw err;

      tinderRecsMen = data.tinderRecs.length;
      console.log('tinderRecsMen: ' + tinderRecsMen);

      calculateRatioFunction(tinderRecsWomen, tinderRecsMen);
    });
  }  

  function calculateRatioFunction(tinderRecsWomen, tinderRecsMen) {
    ...
    updateRatioInDB(passedLocation);
  }
   

  function updateRatioInDB(passedLocation) {
  ...
  getRecsWomenLength(passedLocation, male);
